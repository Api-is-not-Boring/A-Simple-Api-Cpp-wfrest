...
static void not_found(router::HttpResp* resp) {
    resp->set_status(404);
...
static void missing_id(router::HttpResp* resp) {
    resp->set_status(400);
    resp->Json(ordered_json{
...
static void delete_car(router::HttpResp* resp, int id) {
    router::Db::car_delete(id);
    resp->set_status(200);
    resp->Json(ordered_json{
            {"method",     "[v2] -> DELETE with Path Parameter"},
            {"deleted_id", id},
            {"total",      router::Db::count()}
    }.dump());
}
...
void router::ApiRoutes::set_v2_bp(BluePrint& bp)
{
    ...
    bp.GET("/cars/{id}", [](const HttpReq* req, HttpResp* resp) {
        if (req->has_param("id")) {
            ...
            if (Db::car_exists(id)) resp->Json(Db::get_car(id).dump()); else not_found(resp);
        } else missing_id(resp);
    });

    bp.PUT("/cars/{id}", [](const HttpReq* req, HttpResp* resp) {
        if (req->has_param("id")) {
            int id = std::stoi(req->param("id"));
            if (!Db::car_exists(id)) not_found(resp); else {
                Car car = req->json().get<Car>();
                car.id = id;
                Db::car_update(car);
                resp->Json(ordered_json {
                        { "method", "[v2] -> PUT with JSON and Query Parameter" },
                        { "car", car }
                }.dump());
            }
        } else missing_id(resp);
    ...
    bp.DELETE("/cars/{id}", [](const HttpReq* req, HttpResp* resp) {
        if (req->has_param("id")) {
            int id = std::stoi(req->param("id"));
            if (Db::car_exists(id)) delete_car(resp, id); else not_found(resp);
        } else missing_id(resp);
    });
}
    ...
    server.PUT("/api/v2/cars", [](const HttpReq* req, HttpResp* resp) {
        if (req->has_query("id")) {
            ...
            if (Db::car_exists(id)) {
                ...
                Db::car_update(car);
                resp->Json(ordered_json {
                        { "method", "[v2] -> PUT with JSON and Path Parameter" },
                        { "car", car }
                }.dump());
            } else not_found(resp);
